    ...
    <div id="ipl-fixtures" className="crick-container flex flex-col gap-10 pt-10 pb-8 relative z-10 w-full">
      ...
      <div className="flex flex-col items-center justify-center gap-3 text-center px-4 md:px-0 mt-4 md:mt-8">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-3xl md:text-[2.75rem] font-black text-slate-800 uppercase tracking-tight">
            IPL 2026 FIXTURES
          </h2>
        </div>
        ...
        {FIXTURES_DATA.map((match) => (
          <div key={match.id} className="flex flex-col md:flex-row gap-6 md:gap-10 relative mb-6 group">
            ...
            <div className="w-full md:w-36 flex flex-col items-start md:items-end text-left md:text-right pt-2 md:pt-4 relative z-10 pl-6 md:pl-0 shrink-0">
               ...
               <div className="absolute left-0 top-6 w-[1px] h-[calc(100%+1.5rem)] bg-slate-200 md:hidden" />
               <div className="absolute left-[-4px] top-[22px] w-2.5 h-2.5 rounded-full border-2 border-crick-blue bg-white md:hidden z-20" />

               <div className="bg-crick-blue/10 border border-crick-blue/30 text-crick-blue px-3 py-1 rounded text-[9px] font-black uppercase tracking-widest w-fit md:ml-auto shadow-sm">
                 {match.matchLabel}
               </div>
               <h3 className="text-[15px] md:text-[17px] font-black text-slate-800 uppercase mt-4 tracking-wide">
                 {match.date}
               </h3>
            ...
            <div className="hidden md:flex flex-col items-center relative w-8 shrink-0">
               <div className="absolute top-6 bottom-[-2rem] w-[1px] bg-slate-200" />
               <div className="w-2.5 h-2.5 rounded-full border-[2px] border-crick-blue bg-white absolute top-5 z-10 shadow-[0_0_8px_rgba(197,165,100,0.3)] group-hover:scale-150 transition-all duration-300" />
            </div>

            {/* Right Card */}
            <div className="flex-1 bg-white border border-slate-100 rounded-2xl p-4 md:p-6 shadow-sm hover:border-crick-blue/30 transition-all cursor-pointer hover:shadow-md ml-6 md:ml-0">
               ...
               <div className="flex flex-col md:flex-row items-center justify-between gap-5 md:gap-6">
                 ...
                 <div className="flex w-full md:w-auto items-center justify-between md:justify-start gap-1 sm:gap-4 md:gap-8 flex-1 overflow-hidden">
                    ...
                    <div className="flex items-center gap-1.5 md:gap-3 flex-1 w-[45%] md:w-auto min-w-0">
                       <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-slate-50 flex items-center justify-center shrink-0">
                         <img src={match.team1.logo} alt="" className="w-full h-full object-contain" referrerPolicy="no-referrer" />
                       </div>
                       <span className="font-bold text-[10px] sm:text-[13px] md:text-[14px] text-slate-700 leading-[1.15] md:leading-tight block shrink min-w-0 break-words line-clamp-3">
                         {match.team1.name}
                       </span>
                    </div>

                    <span className="text-slate-300 font-black italic text-xs sm:text-base md:text-lg opacity-60 px-0.5 sm:px-1 shrink-0">V/S</span>
